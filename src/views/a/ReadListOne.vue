<template>
	<div class="read-o">
		<div class="w">
			<div class="navi">
				<div class="x">
					<span><v-icon name="microchip"></v-icon>Node <b>{{ readings.chipid }}</b></span>
				</div>
				<div class="y">
			...
			<div class="data">
				<div class="list">
					...
					<div v-if="readings.read.length > 0" class="g">
				...
				<div class="page">
					<div>
						...
						<button :disabled="readings.read.length < listerLimit" @click="nextList()">NEXT<v-icon name="arrow-right"></v-icon></button>
					</div>
					<div class="ta-r">
...
export default {
	...
	methods: {
        fetchReadings() {
			...
			this.$http.get('/node/'+ this.$route.query.nid +'/?action=lister&node_fields=id,name,chipid,read&read_fields=volt,amps,created_at,updated_at&queryString='+ this.queryString +'&listerStart='+ this.listerStart +'&listerLimit='+ this.listerLimit).then(res => {
...
.w .list .h, .w .list .g .i { display: grid; grid-template-columns: 32px 80px 80px 340px auto ; }
